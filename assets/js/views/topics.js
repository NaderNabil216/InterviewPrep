...
export function renderTopics(el, { snapshot, query }) {
  ...
  el.innerHTML = `
    ...
    <p class="muted">${filtered.length} of ${browsableItems.length} items match your filters.</p>
  ...
  el.querySelector('#f-q').addEventListener('input', (e) => { state.q = e.target.value; rerenderList(); });
